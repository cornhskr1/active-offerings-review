...
def get_latest_catalog_url():
    ...
    if not candidates:
        raise RuntimeError("Could not locate the NRGC Authorized Sports Wagering Menu.")
...
def download_pdf(url):
    ...
    if "pdf" not in response.headers.get("content-type", "").lower() and not url.lower().endswith(".pdf"):
        raise RuntimeError("Catalog link did not return a PDF.")
...
def main():
    ...
    previous_lookup = {item.casefold() for item in previous_entries}
