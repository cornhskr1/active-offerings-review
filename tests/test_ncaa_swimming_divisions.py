"""NCAA Division I men's and women's swimming require distinct identities."""
...
class NCAASwimmingDivisionIdentityTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def approval(self):
        sport = next(
            item for item in self.season["sports"] if item["sport"] == "NCAA Swimming"
        )
        return next(
            event
    ...
    def test_legal_parent_has_two_distinct_children(self):
        ...
        self.assertEqual(
            {"Division I Swimming | Men", "Division I Swimming | Women"},
