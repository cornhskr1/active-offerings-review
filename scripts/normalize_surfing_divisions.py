#!/usr/bin/env python3
"""Split combined Surfing approvals into separate operational identities.

The published approval text remains the legal parent. Each child represents
one division-specific competition identity and remains fail-closed for schedule
coverage until that division has independently verified evidence.
"""
...
def normalize(payload):
    surfing = next(s for s in payload["sports"] if s["sport"] == "Surfing")
